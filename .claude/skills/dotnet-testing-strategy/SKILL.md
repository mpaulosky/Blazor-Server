...
description: "Deciding how to test .NET code. Unit vs integration vs E2E decision tree, test doubles, Blazor component testing with bUnit."
---
...
Testing a Razor/Blazor component (markup, parameter binding, event callbacks, cascading state) is its own branch: add a **Component Test with bUnit** — see [Component Testing (bUnit)](#component-testing-bunit) below. bUnit is additive, not a replacement for Playwright: it gives fast, isolated feedback on one component's rendering logic in the same way a unit test gives fast feedback on one class, but it never loads a browser, so it cannot verify a component actually works end-to-end (JS interop, CSS layout, routing, focus/scroll, real user flows). Critical user flows still need Playwright/E2E coverage per the existing criteria below, regardless of how much bUnit coverage exists for the components involved.
...
For Blazor components, use [bUnit](https://bunit.dev) rather than a full browser. It renders components in-memory against the Blazor render tree, so tests run at unit-test speed while still exercising real Razor markup, parameter binding, event callbacks, and cascading values.

**Use bUnit when:**

- Verifying markup renders correctly given a set of parameters
- Verifying an event callback (`onclick`, `@bind`, custom `EventCallback<T>`) fires with the right arguments
- Verifying conditional rendering, cascading parameters, or component lifecycle (`OnInitializedAsync`, `OnParametersSetAsync`)
- Verifying a component reacts correctly to injected service state (stub the service, assert on rendered output)

**bUnit complements Playwright/E2E, it doesn't replace it.** Keep Playwright coverage for anything that depends on the real browser: JS interop, CSS layout, focus/scroll behavior, or navigation across multiple pages -- bUnit never loads a browser, so none of that is observable through it. A component with a Playwright test covering its critical user flow can still benefit from bUnit tests for its edge cases and parameter permutations; write both where the component's risk warrants it.
...
Inherit the test class from bUnit's `TestContext` (or use `IClassFixture<TestContext>` if the class already has another base). Register stubbed services on `Services` in Arrange the same way you would configure a DI container:
...
**Project naming:** `*.ComponentTests`, mirroring the `.UnitTests`/`.IntegrationTests` convention (see [Test Organization](#test-organization) below) -- e.g. `MyApp.Web.ComponentTests`.

**Gotchas:**

- Don't assert on internal component state or private fields -- assert on rendered markup (`cut.Find(...)`, `cut.Markup`) the way a user or a Playwright test would observe it. This keeps bUnit tests aligned with the "test behavior, not implementation" principle used everywhere else in this skill.
- `RenderComponent<T>` throws if a required cascading parameter or injected service isn't registered -- register everything the component actually resolves, not everything it could theoretically need.
- Re-render after changing parameters with `cut.SetParametersAndRender(...)` rather than creating a second `RenderComponent` call; a fresh render doesn't exercise `OnParametersSetAsync` the way a real re-render does.
...
- `*.ComponentTests` -- Blazor component tests via bUnit (see [Component Testing (bUnit)](#component-testing-bunit) above)
