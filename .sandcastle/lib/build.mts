...
// main, the reviewer runs in the same sandbox, and the branch is pushed and
// gets a pull request that closes its issue. Returns the PR's URL, or
// undefined when the branch holds nothing to publish.
export async function buildIssue(
  ...
  try {
    ...
    return {
      ...
      prUrl: publish(issue, branch, sandbox.worktreePath, reviewed),
    };
  } finally {
    await sandbox.close();
  }
