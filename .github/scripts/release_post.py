...
def render_post(pr, tag, merged_date, commits, files, summary, model):
    number = pr["number"]
    title_line = (pr.get("title") or "").strip() or f"PR #{number}"
...
def sort_posts(posts):
    # Same-day posts are ordered by release version, which follows merge order.
    return sorted(posts, key=lambda p: (p["date"], version_key(p["tag"])), reverse=True)
...
def release_entries(gh, repository, tag, merged_date, pr_number, title_line, blog_dir):
    ...
    entries.sort(key=lambda e: (e["date"], version_key(e["tag"])), reverse=True)
...
def render_blogs_html(posts, repository):
    rows = [
        ...
        for p in sort_posts(posts)[:TABLE_SIZE]
    ]
...
def run(repository, pr_number, tag, gh, root=Path("."), api_key=None, model=DEFAULT_MODEL, urlopen=urllib.request.urlopen):
    ...
    (blog_dir / post_name).write_text(
        render_post(pr, tag, merged_date, commits, files, summary, model), encoding="utf-8"
    )
